...
--8(A). Display names of students who belong to either ‘RAJKOT’ or ‘SURAT’ city (USE OR & IN).
	SELECT SNAME FROM STUDENT
	WHERE CITY='RAJKOT' OR CITY='SURAT'

--8(B).Display names of students who belong to either ‘RAJKOT’ or ‘SURAT’ city (USE OR & IN).
...
--10(A). Find all students whose SPI is greater than or equal to 7.0 and less than or equal to 9.0 (USE AND & BETWEEN). 
	SELECT * FROM STUDENT
	WHERE SPI >= 7.0 AND SPI <= 9.0;

--10(B). Find all students whose SPI is greater than or equal to 7.0 and less than or equal to 9.0 (USE AND & BETWEEN). 
	SELECT * FROM STUDENT
...
--14(A). Display all student names other than ‘DEEP’ from STUDENT table (USE NOT, <>, !=). 
	SELECT SNAME FROM STUDENT
	WHERE NOT SNAME = 'DEEP';

--14(B). Display all student names other than ‘DEEP’ from STUDENT table (USE NOT, <>, !=). 
	SELECT SNAME FROM STUDENT
	WHERE SNAME <> 'DEEP';

--14(C). Display all student names other than ‘DEEP’ from STUDENT table (USE NOT, <>, !=). 
	SELECT SNAME FROM STUDENT
	WHERE SNAME != 'DEEP';
